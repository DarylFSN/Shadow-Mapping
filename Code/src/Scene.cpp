...
void Scene::draw(bool isLight){
    ...
    if (isLight) {
        ...
        std::stack <mat4>  matrix_stack_LS;
        ...
        mat4 VM_LS = glm::lookAt(vec3(sun->position), glm::vec3(0.0f, 1.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        ...
        while (!dfs_stack.empty()) {
            ...
            VM_LS = matrix_stack_LS.top();  matrix_stack_LS.pop();
            ...
            for (unsigned int i = 0; i < cur->models.size(); i++) {
                ...
                shader->modelviewLS = VM_LS * cur->modeltransforms[i];
            ...
            for (unsigned int i = 0; i < cur->childnodes.size(); i++) {
                ...
                matrix_stack_LS.push(VM_LS * cur->childtransforms[i]);
            }

        } // End of DFS while loop.
        // HW3: Your code will only be above this line.
    } else {
        glUseProgram(sun->shader->program);
        ...
        // Initialize the current state variable for DFS
        Node* cur = node["world"]; // root of the tree
        mat4 cur_VM = glm::lookAt(glm::vec3(sun->position), glm::vec3(0.0f, 1.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        ...
        while (!dfs_stack.empty()) {
            ...
            for (unsigned int i = 0; i < cur->models.size(); i++) {
                ...
                // (HW3 hint: you should do something here)
                sun->shader->modelview = cur_VM * cur->modeltransforms[i]; // HW3: Without updating cur_VM, modelview would just be camera's view matrix.

                // The draw command
                sun->shader->setUniforms();
                cur->models[i]->geometry->draw();
            }
